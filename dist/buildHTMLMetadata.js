...
const buildHTMLMetadata = (production = {
  ...
  ` : '';
  const authors = production.metadata.authors && production.metadata.authors.length ? production.metadata.authors.map(author => `
                    <meta name="DC.Creator" content="${`${author.given} ${author.family}`}" />
                    <meta name="author" content="${`${author.given} ${author.family}`}" />`) : ''; // todo: use cover image and convert it to the right base64 dimensions for the social networks
...
var _default = buildHTMLMetadata;
